    ...
    fun editTrip(
        ...
        database.collection("users").document(uId).collection("trips")
            ...
            .addOnSuccessListener { documentReference ->
                Toast.makeText(
                    context,
                    "Edited trip on ${trip.dateTime} with ${trip.trainBrand} ${trip.trainName}",
                    Toast.LENGTH_LONG
                ).show()
                Log.d(
                    TAG, "DocumentSnapshot edited with ID: $documentReference"
                )
                Log.d(
                    TAG, "Train edited: " + trip.dateTime + trip.trainBrand + trip.trainName
                )
            }
            .addOnFailureListener { e ->
                Log.w(
